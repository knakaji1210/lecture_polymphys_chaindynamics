...
import singleChainDynamicsFunc_Reptation as scd
...
try:
    radi = int(input('Radius of Tube (default=20): '))
except ValueError:
    radi = 20
...
ax.hlines(radi, 0, plot_lim, color="red")
ax.hlines(-radi, 0, plot_lim, color="red")
ax.vlines(0, radi, plot_lim, color="red")
ax.vlines(0, -radi, -plot_lim, color="red")
...
if initConfig == "F": # Fully Extendedからスタートする場合
    savefile = "./gif/SingleChain_Dynamics_N{0}_{1}steps_FE_Rep".format(N, t_max)
if initConfig == "R": # Fully Extendedからスタートする場合
    savefile = "./gif/SingleChain_Dynamics_N{0}_{1}steps_RC_Rep".format(N, t_max)
