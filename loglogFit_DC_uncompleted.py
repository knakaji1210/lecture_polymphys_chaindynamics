# 重心の拡散係数の重合度依存性
# エラーを考慮するようにしたが、log(tau +- del_tal) = log(tau) +- del_tal/tauで
# 定義するものはフィッティングのエラーなので、本当の意味でのエラーではない
# 一方、例えばフィッティングできる程度に繰り返し数を減らして、その繰り返しを行うように
# プログラムを変更できるとは思うが・・・
# このプログラムで出てくる結果は、むしろNの大きい方で重みが高くなるので、
# 見た目のフィッティング結果は悪くなる。
# そういうわけで、このプログラムは安定バージョンとしない

import numpy as np
from scipy.optimize import curve_fit
import matplotlib.pyplot as plt

DP_val = input('Enter DP values separated by comma: ')
DC_val = input('Enter DC values separated by comma: ')
DC_err_val = input('Enter DC err values separated by comma: ')

DP_val = DP_val.split(',')
DP_list = [ float(val) for val in DP_val ]
logDP_list = [ np.log10(DP) for DP in DP_list ]

DC_val = DC_val.split(',')
DC_list = [ float(val) for val in DC_val ]
logDC_list = [ np.log10(DC) for DC in DC_list ]

DC_err_val = DC_err_val.split(',')
DC_err_list = [ float(val) for val in DC_err_val ]
logDC_err_list = [ DC_err/DC for DC_err, DC in zip(DC_err_list, DC_list)]   # 誤差伝播

def loglogFit(x, a, b):
    return  a*x + b

param, cov = curve_fit(loglogFit, logDP_list, logDC_list, sigma=logDC_err_list, absolute_sigma=True)
slope = param[0]
err_slope = np.sqrt(cov[0][0])
logDC_fit_list = [ loglogFit(logDP, param[0], param[1]) for logDP in logDP_list ]

resultText = "$D$  ∝ $N^{{{{{0:.3f}}}±{{{1:.3f}}}}}$".format(slope, err_slope)

fig = plt.figure(figsize=(8,8))

ax = fig.add_subplot(111, title='Scaling of Diffusion Const, $D$', 
            xlabel='Log($N$)', ylabel='Log($D$)')
ax.grid(visible=True, which='major', color ='#666666', linestyle='--')

ax.errorbar(logDP_list, logDC_list, yerr = logDC_err_list, capsize=5, fmt='o', markersize=6, ecolor='black', color='r')
# ax.scatter(logDP_list, logDC_list, marker='o', s=50, c='red')
ax.plot(logDP_list, logDC_fit_list,  c='blue')

fig.text(0.65, 0.70, resultText)

savefile = "./png/Scaling_DiffusionConst"
fig.savefig(savefile, dpi=300)

plt.show()
plt.close()